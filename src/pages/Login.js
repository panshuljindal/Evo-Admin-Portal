...
function Login() {
  ...
  const handleSubmit = () => {
      axios.post("{{BASE_URL}}/club/login", {
        email: email,
        password: password
      }).then(res => {
        if(res.status === 200){
    ...
    <div className="main">
      ...
      <div className="right-login">
        <div className="input-box">
          ...
          <InputBox type="email" place="Email" value={email} onChange={(e) => {setEmail(e.target.value)}} />
          <InputBox type="password" place="Password" value={password} onChange={(e) => {setPassword(e.target.password)}} />
          <br/>
